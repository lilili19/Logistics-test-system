...
function sele() {
    var body = '<tr><th><a href="###" onclick="sele()">热门</a></th><th><a href="###" onclick="sbo.go(\'country\', \'#txtCountryNo\', \'c1\')">国家</a></th></tr>';
    body += '<tr><td>123</td><td>123</td></tr>'
        +'<tr><td>123</td><td>123</td></tr>'
        +'<tr><td>123</td><td>123</td></tr>';
    $('#tabSearchBox').html(body);
    $('#divSearchPage').hide();
} 

function SearchBox(){
    this.host = "http://192.168.1.116:85";
    this.url = "";
    this.TabData = "";
    this.myMethod = null;
    this.rsCount = 0;
    ...
    this.go = function(method,id,valName,e,key,data,myMethod){
        this.go2(method,id,valName,null,null,e, key,data,myMethod);
    };

    //loca位置控件 method-方法   obj1-容器1   v1选字段1    obj2-容器2    v2选字段2 data-参数  key-关键字
    this.go2 = function(method,id,valName,id1,valName1,e,key,data,myMethod){
        var obj = null, obj1 = null;
        if(id.indexOf("parent.")>-1 && e!=undefined && e!=null && e!=""){
            id = id.substring(7);
            obj =  $(e).parent().find("[aplDataName="+id+"]");
        ...
        if(id1!=undefined && id1!=null && id1!="") {
            if (id1.indexOf("parent.")>-1 && e!=undefined && e!=null && e!="") {
                id1 = id1.substring(7);
                obj1 = $(e).parent().find("[aplDataName=" + id1 + "]");
            }
            else if (typeof (id1) == "object")
                obj1 = id1;
            else
                obj1 = $(id1);
        }
    ...
    this.show = function(resData){
        ...
        if(resData != undefined && resData != null && resData.rowCount != undefined)
            rsCount = resData.rowCount - 0;
        else
            rsCount = resData.rows.length;
        ...
        if(SearchBox.my.IsShow == 0){
            ...
            strHtml += '    <tbody><tr></tr></tbody> </table> ';
            strHtml += '<div id="divSearchPage"> </div></div> </div>';

            //弹出框设置
            SearchBox.my.layerIndex = window.layer.open({
                type: 1,
                shift: 2,
                //skin : 'layui-layer-demo', //样式类名
                shadeClose: true, //开启遮罩关闭
        ...
        if (mothod == "country") //国家
            SearchBox.my.cols = "c1,国家,280|c3,,0";
        else if (mothod == "coagent") //服务商
            SearchBox.my.cols = "c1,服务商编号,80|c3,服务商名称,200";
        else if (mothod == "region") //分区表
            SearchBox.my.cols = "c1,渠道类型,80|c2,分区表名称,200";
        ...
        for(var colIndex in myCols){
            ...
            if (myCol[1] != "") trBody += "<th style=\"width:" + myCol[2] + "px;\">" + '<a href="###">' + myCol[1] + '</a>' + "</th>";
        }
        trBody = '<th><a href="###" onclick="sele()">热门</a></th>' +　trBody;
...
SearchBox.showTab = function(resData){
    ...
    $(resData.rows).each(function(rowIndex,dr){
        ...
        for(var colIndex in myCols){
            myCol = myCols[colIndex].split(',');
            colName = myCol[0];
            // alert("colName="+dr[colName]);
            if (v%2 == 0) {
                v++;
                trBody = dr[colName];
            }else{
                v++;               
                trBody += "<td colspan='2' style=\"text-align:center;width:" + myCol[2] + "px\"><a onclick=\"SearchBox.showData(" + rowIndex + ")\">" + trBody + dr[colName] + "</a></td>"; 
                // alert(trBody)
                if(trBody != ""){
                    trBody = "<tr>" + trBody + "</tr>";
...
SearchBox.showData = function(i){
    ...
    window.layer.close(SearchBox.my.layerIndex);
